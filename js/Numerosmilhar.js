...
function enviarDadosParaFirestore(email, corretas, incorretas) {
    db.collection(firebase.auth().currentUser.email).doc('Resposta Milhar').set({
...
function recuperarDadosDoFirestore(email) {
    db.collection(email).doc('Resposta Milhar').get()
    .then(function(doc) {
        if (doc.exists) {
...
function gerardezena() {
    var dezena = Math.floor(Math.random() * 9000) + 1000;
